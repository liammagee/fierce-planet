...
Memory.prototype.addVisit = function(age) { this._age = age; this._visits++; }
...
function Agent(level, agentType, x, y) {
    ...
    this._memoriesOfUntriedPaths = new Array();
...
Agent.prototype.memorise = function(x, y) {
    ...
    if (this._memoriesOfPlacesVisited[[x, y]] != undefined) {
        ...
        memory.addVisit(this._age);
    }
    else {
        memory = new Memory(this._age, x, y);
        this._memoriesOfPlacesVisited[[x, y]] = memory;
    }
    if (this._memoriesOfUntriedPaths[[x, y]] != undefined) {
        this._memoriesOfUntriedPaths[[x, y]] = undefined;
//        console.log("Undefining untried path: " + [x, y]);
    ...
    if (x - 1 >= 0 && this._level.getTile(x - 1, y) == undefined && this._memoriesOfPlacesVisited[[x - 1, y]] == undefined) {
        ...
        this._lastUntriedPathMemory = new Memory(this._age, x - 1, y);
        this._memoriesOfUntriedPaths[[x - 1, y]] = this._lastUntriedPathMemory;
...
//        console.log("Defining untried path: " + [x + 1, y]);
    ...
    if (y - 1 >= 0 && this._level.getTile(x, y - 1) == undefined && this._memoriesOfPlacesVisited[[x, y - 1]] == undefined) {
        // Add path cell to memory
        this._lastUntriedPathMemory = new Memory(this._age, x, y - 1);
        this._memoriesOfUntriedPaths[[x, y - 1]] = this._lastUntriedPathMemory;
...
//        console.log("Defining untried path: " + [x, y + 1]);
    ...
    for (var i = 0; i < agents.length; i++) {
        ...
        if (agentX == x && agentY == y) {
            // Add agent to memory
            this._memoriesOfAgents[[x, y]] = agent;
        }
    }
...
Agent.prototype.findPosition = function(withNoRepeat, withNoCollision, withOffscreenCycling) {
    ...
    for (var i = 0; i < candidateCells.length; i++) {
        ...
        if (this.isGoalCell(candidate[0], candidate[1]))
            return candidate;
    ...
    if (candidatesNotInHistory.length > 0) {
        ...
        for (var l = 0; l < agents.length; l++) {
            ...
            if (agent._x == this._x && agent._y == this._y) {
                console.log("Meeting an agent");
                var hasUnsharedUnvisitedMemory = true;
                for (var k in agent._memoriesOfUntriedPaths) {
                    var otherAgentMemory = agent._memoriesOfUntriedPaths[k];
                    if (otherAgentMemory == undefined)
                        continue;

                    var ownAgentMemory = this._memoriesOfUntriedPaths[k];
                    if (ownAgentMemory == undefined) {
                        hasUnsharedUnvisitedMemory = true;
                        break;
                    }
                }
                if (! hasUnsharedUnvisitedMemory) {
                    hasExhaustedAllPaths = true;
                    break;
                }
            }
        }
        if (hasExhaustedAllPaths) {
            console.log("Exhausted all paths");
            return [lastX, lastY];
        }
        ...
        if (candidateCells.length > 1) {
            ...
            for (var i = 0; i < candidateCells.length; i++) {
                ...
                for (var j in this._memoriesOfUntriedPaths) {
                    var unvisited = this._memoriesOfUntriedPaths[j];
...
//                console.log("Untried tiles: " + counter);
//                console.log("Untried tiles undefined: " + counterUndefined);
                if (shortestAgeDifferenceToCandidate > -1 && (shortestAgeDifference == -1 || shortestAgeDifferenceToCandidate < shortestAgeDifference)) {
                    ...
                    bestCandidate = candidate;
//                    console.log(shortestAgeDifference);
                }
                    /*
                var distanceToUntriedPath = mostRecentMemoryOfCandidate.getDistanceFromLastUntriedPath();
                if (distanceToUntriedPath > -1 && (closestToUntriedPath == -1 || distanceToUntriedPath < closestToUntriedPath)) {
                    closestToUntriedPath = distanceToUntriedPath;
                    bestCandidate = candidate;
//                    console.log(closestToUntriedPath);
                }
                */
            }
...
Agent.prototype.getClosestUnvisitedTile = function(currentX, currentY) {
    for (var unvisited in this._memoriesOfUntriedPaths) {
...
Agent.prototype.isGoalCell = function(x, y) {
